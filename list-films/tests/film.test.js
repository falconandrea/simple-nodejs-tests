...
describe('Test front-end endpoints', () => {
  ...
  it('add new film', async () => {
    const res = await request.post('/new-film')
  ...
  it('delete a film', async () => {
    ...
    const res = await request.post(`/delete-film/${film.id}`)
      .expect(response => {
        expect(response.status).toBe(200)
  ...
  it('delete a film with wrong id', async () => {
    const res = await request.post('/delete-film/62bc8747de2202dba8904293')
      .expect(response => {
        expect(response.status).toBe(500)
